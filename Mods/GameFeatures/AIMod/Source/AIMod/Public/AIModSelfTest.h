 ...
 * automatically whenever a game world finishes loading, so AIMod
 ...
 *
 * Add a new Check*() function here whenever new AIMod functionality is
 * added, and call it from RunAll(), rather than only documenting a
 * manual verification step for it.
 *
 * Compiled out of Shipping builds entirely (see AIMod.cpp) - this is a
 * development-time convenience, not something that should run for
 * players of a released mod.
 */
namespace AIModSelfTest
{
	/** Runs all self-tests against the given world and logs a summary via LogAIModAI. Safe to call on any game world, including a real save. No-op if World is null. */
