...
#include "DocModDeveloperSettings.generated.h"

/**
 * DocMod dev-time settings (PLAN.md - this is a development convenience,
 * see DocModHotkey.h). Editable via Project Settings > Plugins > DocMod
 * in the Editor, or by hand-editing Config/DefaultDocMod.ini
 * ([/Script/DocMod.DocModDeveloperSettings], HotkeyKey=(KeyName="F6")) -
 ...
 */
UCLASS(config = DocMod, defaultconfig, meta = (DisplayName = "DocMod"))
class DOCMOD_API UDocModDeveloperSettings : public UDeveloperSettings
{
	GENERATED_BODY()

public:
	UDocModDeveloperSettings();

	/**
	 * Key that triggers DocModHotkey (print targeted manufacturer + test
