# AIMod RPC reference (generated)

Auto-generated from the dispatcher by `controller/tools/gen_rpc_catalog.py`. **104 methods.** The running mod serves this same catalog live via the `world.help` RPC (returns the full catalog; filter client-side). Param *types* can also be confirmed at runtime from structured errors like `params.buildableId must be a non-empty string`. Deep placement guidance: `docs/factory-placement-guide.md`, `docs/vehicle-placement-guide.md`.
